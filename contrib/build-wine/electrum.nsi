...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://www.dogecoin.org/"
  !define PRODUCT_PUBLISHER "Dogecoin Project"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-doge\electrum_doge\electrum\gui\icons\electrum_doge.ico"
...
Section
  ...
  File "c:\electrum-doge\electrum_doge\electrum\gui\icons\electrum_doge.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-doge-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-doge-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-doge-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-doge-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-doge-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\dogecoin" "DefaultIcon" "$\"$INSTDIR\electrum_doge.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\dogecoin\shell\open\command" "" "$\"$INSTDIR\electrum-doge-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum_doge.ico"
