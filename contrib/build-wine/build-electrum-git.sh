...
pushd $WINEPREFIX/drive_c/electrum-doge
...
for i in ./locale/*; do
    dir=$WINEPREFIX/drive_c/electrum-doge/electrum_doge/electrum/$i/LC_MESSAGES
    mkdir -p $dir
...
cd dist
mv electrum-doge-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
...
sha256sum dist/electrum-doge*.exe
